...
function File(params) {
    ...
    switch(this.resolvedPath.type){
        case Path.FILE_TYPE.ASSET:
            ...
            if(!nativeAssetsList){
                nativeAssetsList = array(activity.getAssets().list(""));
            }

            nativeAssetsList.forEach(function(assetName) {
                if (this.resolvedPath.name === string(assetName)) {
        ...
        case Path.FILE_TYPE.DRAWABLE:
            ...
            this.drawableResourceId = resources.getIdentifier(string(this.resolvedPath.name), string("drawable"), string(AndroidConfig.packageName));
            this.nativeObject = this.drawableResourceId !== 0 ? NativeBitmapFactory.decodeResource(resources, int(this.drawableResourceId)) : null;
            break;
        ...
        case Path.FILE_TYPE.FILE:
            ...
            this.nativeObject = this.resolvedPath.fullPath ? new NativeFile(string(this.resolvedPath.fullPath)) : null;
            break;
...
File.prototype = {
    ...
    get exists(){
        if(this.resolvedPath.type === Path.FILE_TYPE.DRAWABLE || this.resolvedPath.type === Path.FILE_TYPE.ASSET){
            ...
            return this.nativeObject && bool(this.nativeObject.exists());
        }
    },
    get extension(){
