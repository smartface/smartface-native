...
const SYSTEM_UI_FLAG_LIGHT_STATUS_BAR =
	NativeView.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
...
export default class StatusBar {
	...
	static set style(value: StatusBarStyle) {
		if (
			NativeBuildVersion.VERSION.SDK_INT >= MINAPILEVEL_STATUSBARICONCOLOR
		) {
	...
	static get height(): number {
		let result = 0;
		const resourceId = AndroidConfig.activityResources.getIdentifier(
			'status_bar_height',
			'dimen',
			'android'
		);
		if (resourceId > 0) {
			result =
				AndroidConfig.activityResources.getDimensionPixelSize(
					resourceId
				);
		}
	...
	static get android() {
		return {
			...
			set color(value: Color) {
				...
				if (
					NativeBuildVersion.VERSION.SDK_INT >=
					MINAPILEVEL_STATUSBARCOLOR
				) {
					const window = AndroidConfig.activity.getWindow();
					window.addFlags(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
					// TODO: color needs nativeObject
