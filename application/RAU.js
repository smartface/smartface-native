...
function checkUpdateFromCache(callback, userInfo) {
  ...
  sessionManager.request({
    url:
      "https://portalapi.smartface.io/api/v1/rau/check?v=" +
      Math.floor(Math.random() * 100000), // to avoid response cache
    method: "POST",
    ...
    onLoad: function(response) {
      if (response.statusCode === 200) {
        ...
        // No update for Android
        callback("No update", null);
      } else if (response.statusCode === 204) {
        // There is update but not found in cache
...
function download(callback) {
  ...
  sessionManager.request({
    ...
    onLoad: function(response) {
      ...
      var zipFileStream = zipFile.openStream(
