...
const EventFunctions = {
	...
	[Events.NotificationReceive]: function () {
		this.onCallStateChanged = function (state) {
			this.emitter.emit(Events.NotificationReceive, state);
		};
		if (
			__SF_UNUserNotificationCenter.currentNotificationCenter().delegate ===
			undefined
		) {
			__SF_UNUserNotificationCenter.currentNotificationCenter().delegate =
				Notifications.ios.UNUserNotificationCenterDelegate;
		}
...
Notifications.LocalNotification = function LocalNotification(params) {
	...
	this.schedule = function () {
		__SF_UIApplication
			.sharedApplication()
			.scheduleLocalNotification(self.nativeObject);
	};

	this.present = function () {
		__SF_UIApplication
			.sharedApplication()
			.presentLocalNotificationNow(self.nativeObject);
	};

	this.cancel = function () {
		__SF_UIApplication
			.sharedApplication()
			.cancelLocalNotification(self.nativeObject);
	};
...
Object.defineProperties(Notifications.ios, {
	applicationIconBadgeNumber: {
		...
		set: function (value) {
			if (typeof value === "number") {
				__SF_UIApplication.sharedApplication().applicationIconBadgeNumber =
					value;
			}
		},
		enumerable: true,
	...
	scheduledLocalNotifications: {
		get: function () {
			...
			var nativeNotifications =
				__SF_UIApplication.sharedApplication().scheduledLocalNotifications;
...
Notifications.registerForPushNotifications = function (onSuccess, onFailure) {
	...
	var userNotificationSettings =
		__SF_UIUserNotificationSettings.settingsForTypesCategories(
			__SF_UIUserNotificationTypeSound |
				__SF_UIUserNotificationTypeAlert |
				__SF_UIUserNotificationTypeBadge,
			undefined
		);
	__SF_UIApplication
		.sharedApplication()
...
Notifications.ios.getAuthorizationStatus = function (callback) {
	var current = Invocation.invokeClassMethod(
	...
	var argBlock = new Invocation.Argument({
		...
		value: function (settings) {
			var status = Invocation.invokeInstanceMethod(
				settings,
				"authorizationStatus",
				[],
				"NSInteger"
...
Notifications.ios.UNUserNotificationCenterDelegate.willPresentNotification =
	function (e) {
...
Notifications.ios.UNUserNotificationCenterDelegate.didReceiveNotificationResponse =
	function (e) {
		Notifications.ios._didReceiveNotificationResponse &&
			Notifications.ios._didReceiveNotificationResponse(e);
	};
